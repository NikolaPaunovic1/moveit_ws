...
public:
    ...
    client_("/franka_gripper/grasp", true)  // Initialize the client in the constructor
  {
    ...
    grasp_dictionary.push_back(create_grasp_data(0, -tau / 2, tau * 5 / 8, -0.036, 0, 0.19, 0.0, 0.0, 0, 0.02, 0.0, 0.22, 0.003));
    //grasp_data for object_id=2 (knife)
    grasp_dictionary.push_back(create_grasp_data(0, -tau / 2, tau * 5 / 8, -0.045, 0.0, 0.2, 0.0, 0.0, 0, -0.035, 0.0, 0.2, 0.013));
    ...
    grasp_dictionary.push_back(create_grasp_data(0, -tau / 2, tau * 5 / 8, -0.045, 0.0, 0.2, 0.0, 0.0, 0, 0.045, 0.0, 0.21, 0.013));
    //grasp_data for object_id=5 (flakes)
    grasp_dictionary.push_back(create_grasp_data(0, -tau / 2, tau * 5 / 8, 0.0, 0.0, 0.14, 0.0, 0.0, 0, 0, 0, 0.26, 0.07));
    //grasp_data for object_id=6 (bowl)
    grasp_dictionary.push_back(create_grasp_data(0, -tau / 2, tau * 5 / 8, 0.045, 0.0, 0.2, 0.0, 0.0, 0, -0.07, 0.0, 0.25, 0.013));
  ...
  while(!found_object){
    ...
    // gripper_group.setStartState(*gripper_group.getCurrentState());
    // float openwidth = (grasp_dictionary[(goal->id)-1].gripper_width/2) + 0.02;
