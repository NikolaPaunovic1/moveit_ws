...
public:
  ...
  while(!found_object){
    ...
    //Hardcode pregrasp pose relative to the object
    tf2::Quaternion quat_grasp;
    quat_grasp.setRPY(0, -tau / 2, tau *5/ 8);
    tf2::Matrix3x3 rot_mat_grasp(quat_grasp);
    tf2::Vector3 translation_grasp(0.0, 0.0, 0.16);
    tf2::Transform T_grasp(rot_mat_grasp, translation_grasp);

    //Initialize Objet Transform
    ...
    //Multiply with object transform to define the PoseTarget = PregraspPose
    geometry_msgs::Pose target_pose1;
    tf2::Transform T_pregrasp = T_object * T_grasp;
  ...
  //Compute desired endeffector pose
    ...
    tf2::Vector3 translation_goal(goal->pose.position.x, goal->pose.position.y, goal->pose.position.z);
    tf2::Transform T_goal(rot_mat_goal, translation_goal);

    //compute transform of place pose
    tf2::Quaternion correct_grasp;
    correct_grasp.setRPY(0.0, 0.0, -tau/4);
    tf2::Matrix3x3 correct_mat(correct_grasp);
    tf2::Matrix3x3 rot_mat_place(correct_mat * rot_mat_grasp);
    tf2::Vector3 translation_place(0, 0, 0.18);
    tf2::Transform T_place(rot_mat_place, translation_place);

    //multiply goal and place transform to get desired endeffector pose
    geometry_msgs::Pose target_pose2;
    T_place = T_goal * T_place;
    target_pose2.position.x = T_place.getOrigin().getX();
    target_pose2.position.y = T_place.getOrigin().getY();
    target_pose2.position.z = T_place.getOrigin().getZ();
    target_pose2.orientation = tf2::toMsg(T_place.getRotation());
