...
int main(int argc, char **argv)
{
    ...
    visual_tools.prompt("Press Next to call the object detection service!");

    ros::ServiceClient service_client = nh.serviceClient<my_services::ObjectPositionService>("object_position_service");

    service_client.waitForExistence();

    my_services::ObjectPositionService srv;

    srv.request.shape_name = "square";
    ...
    //DERIVE OBJECT POSE FROM SERVICE CALL!
    geometry_msgs::Pose object_pose = srv.response.object_pose;
    ...
    //++++++++++++++++++++++++++++++++
    primitive3.dimensions[primitive3.BOX_X] = 0.032;
    primitive3.dimensions[primitive3.BOX_Y] = 0.032;
    primitive3.dimensions[primitive3.BOX_Z] = 0.064;


    //DERIVE THE BOX POSE OUT OF THE OBJECT POSE FROM THE SERVICE CALL
    geometry_msgs::Pose object3_pose;
    object3_pose = object_pose;

    //CALCULATE MIDDLE POINT (IN BETWEEN TABLE HEIGHT AND SERVICE CALL POSITION)
    object3_pose.position.z = (object_pose.position.z - 0.4)/2 + 0.4;
    ...
    //DERIVE THE GRASP POSE FROM THE OBJECT POSITION------------------------------
    ...
    //ADD OFFSET FOR THE GRIPPER
    target_pose1.position.z += 0.073;
    ...
    target_pose2.position.z = object_pose.position.z + 0.1;
    //++++++++++++++++++++++++++++++++++++++++++++++
