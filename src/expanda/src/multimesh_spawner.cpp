...
//define box collision object
moveit_msgs::CollisionObject cube;
cube.header.frame_id = move_group_interface.getPlanningFrame();
...
cube_pose.position.z = 3.0;
// Add the mesh to the CollisionObject
shapes::Mesh* m = shapes::createMeshFromResource("package://expanda/meshes/cube_stick.stl"); //find mesh
...
cube.mesh_poses.push_back(cube_pose);
cube.operation = cube.ADD;


//Plate collision Object
moveit_msgs::CollisionObject plate;
plate.header.frame_id = move_group_interface.getPlanningFrame();
geometry_msgs::Pose plate_pose = cube_pose;
// Add the mesh to the CollisionObject
m = shapes::createMeshFromResource("package://expanda/meshes/plate_330_240.stl"); //find mesh
for (unsigned int i = 0; i < m->vertex_count; ++i)
{
m->vertices[i * 3] *= scale_factor;
m->vertices[i * 3 + 1] *= scale_factor;
...
ros::Duration(0.5).sleep();
mesh = boost::get<shape_msgs::Mesh>(mesh_msg); // shape msg is assigned to mesh msg
plate.meshes.push_back(mesh);
plate.mesh_poses.push_back(plate_pose);
...
geometry_msgs::Pose bowl_pose = cube_pose;
// Add the mesh to the CollisionObject
m = shapes::createMeshFromResource("package://expanda/meshes/bowl.stl"); //find mesh
for (unsigned int i = 0; i < m->vertex_count; ++i)
{
m->vertices[i * 3] *= scale_factor;
m->vertices[i * 3 + 1] *= scale_factor;
...
bowl.operation = plate.ADD;


//fill the bin vector with the collisionobjects you want
bins.push_back(cube); //should correspond to mesh of aruco_id specified first in launchfile
bins.push_back(cube); //should correspond to mesh of aruco_id specified second in launchfile
bins.push_back(bowl); //should correspond to mesh of aruco_id specified third in launchfile
bins.push_back(plate); //should correspond to mesh of aruco_id specified fourth in launchfile
bins.push_back(cube); //should correspond to mesh of aruco_id specified fifth in launchfile
...
for (int i = 0; i < 5; ++i) {
    ...
    marker.counter = 0;
    marker.tf1 = poseToStampedTransform(cube_pose, "world", marker.aruco_frame_long);
    marker.tf2 = poseToStampedTransform(cube_pose, "world", marker.aruco_frame_short);
...
cube.primitives.clear(); //Clear mesh required for MOVE operation (Only to avoid a warning)
cube.operation = cube.MOVE;
plate.primitives.clear(); //Clear mesh required for MOVE operation (Only to avoid a warning)
plate.operation = plate.MOVE;
